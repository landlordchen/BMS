...
function addInputboxDataForSaving(c, d) {
    ...
    var e = null;
    if (b.reportfamily == ReportFamily.EDITABLELIST2 || b.reportfamily == ReportFamily.EDITABLEDETAIL2) {
        e = f
    } else {
...
function addNewDataRow(n, h, w) {
    ...
    for (var y = 0; y < k.length; y = y + 1) {
        ...
        if (t.coltype == "ROWSELECTED-RADIOBOX") {
            setColDisplayValueToEditable2Td(j, '<input type="radio" onclick="doSelectedDataRowChkRadio(this)" name="' + h + '_rowselectbox_col">');
            continue
        } else {
            if (t.coltype == "ROWSELECTED-CHECKBOX") {
                setColDisplayValueToEditable2Td(j, '<input type="checkbox" onclick="doSelectedDataRowChkRadio(this)" name="' + h + '_rowselectbox_col">');
                continue
            }
        }
        ...
        if (F === "true") {
            ...
            if (f != null && (f.getAttribute("parentids") == null || f.getAttribute("parentids") == "")) {
    ...
    if (c.rows.length == 0) {
        ...
        if (x.metaDataSpanObj.getAttribute("addposition") !== "top") {
            ...
            for (var y = 0; y < c.rows.length; y++) {
                ...
                if (s != null && s.indexOf("cls-data-tr") == 0 && s != "cls-data-tr-head-list") {
                    p = c.rows[y];
                    break
                }
            }
...
function getEditableListReportColValues(b, a, c, h) {
    ...
    for (var e = 0, f = g.length; e < f; e = e + 1) {
        var d = wx_getAllColValueByParentElementObjs(g[e].getElementsByTagName("TD"), c);
        if (d != null) {
            j[j.length] = d
        }
    }
...
function preSaveEditableDetail2ReportData(j, a) {
    ...
    if (a == WX_SAVETYPE_DELETE) {
        ...
        if (c == "changed" && (WX_UPDATE_ALLDATA == null || WX_UPDATE_ALLDATA[d] == null || WX_UPDATE_ALLDATA[d] == "")) {
            return WX_SAVE_IGNORE
        }
        f.WX_TYPE = "update"
    }
...
function getEditable2ColRealValueByFormatemplate(b, c, g, a, e) {
    if ((e == null || e == "") && (g.indexOf("[") != 0 || g.lastIndexOf("]") != g.length - 1)) {
        return e
    }
    ...
    if (n.reportfamily == ReportFamily.EDITABLELIST2) {
        ...
        if (n.reportfamily == ReportFamily.EDITABLEDETAIL2) {
            datasObj = getEditableReportColValues(n.pageid, n.reportid, null, null)
        } else {
            return e
        }
    }
    ...
    for (var h = 0; h < j.length; h++) {
        ...
        f = j[h].substring(m + 1);
        l = f.lastIndexOf("__old") > 0 ? f.substring(0, f.lastIndexOf("__old")) : f;
        if (datasObj[l] == null) {
            o = ""
        } else {
            if (l != f && datasObj[l].oldname != null && datasObj[l].oldname != "") {
...
function closeMeAndRefreshParentReport(b) {
    ...
    if (WXConfig.prompt_dialog_type == "ymprompt") {
        parent.refreshParentEditableListReport(b.pageid, b.reportid, b.edittype, a)
    } else {
        artDialog.open.origin.refreshParentEditableListReport(b.pageid, b.reportid, b.edittype, a)
    }
}
...
function saveEditableReportDataImpl(v) {
    ...
    for (var r = 0, s = e.length; r < s; r = r + 1) {
        ...
        if (o == null) {
            wabacus_info("\u6ca1\u6709\u53d6\u5230\u9875\u9762ID\u4e3a" + k + ",\u62a5\u8868ID\u4e3a" + j + "\u7684\u5143\u6570\u636e\uff0c\u65e0\u6cd5\u5bf9\u5176\u8fdb\u884c\u4fdd\u5b58\u64cd\u4f5c\uff0c\u53ef\u80fd\u6b64\u62a5\u8868\u6ca1\u6709\u663e\u793a\u51fa\u6765");
            continue
        }
    ...
    if (a > 1 && m != "") {
        d = replaceUrlParamValue(d, "refreshComponentGuid", "[DYNAMIC]" + m, true)
    }
    if (!hasSaveData(d) && (WX_listReportDeleteInfo == null || WX_listReportDeleteInfo.length == 0)) {
        if (l === true && h === true) {
            wx_warn("\u6ca1\u6709\u8981\u4fdd\u5b58\u548c\u5220\u9664\u7684\u6570\u636e")
        } else {
            if (l === true) {
...
function getRealSavingUrl(b, d, c) {
    ...
    if (b.slave_reportid == b.reportid || b.metaDataSpanObj.getAttribute("isSlaveDetailReport") == "true") {
        if (e.add == true) {
...
function doSaveEditableReportData(h, g) {
    var a = (g != null && g.indexOf("|") > 0) ? g = g.substring(0, g.indexOf("|")) : g;
    ...
    if (i != null && i.method != null) {
        var j = i.method(h.pageid, h.reportid, WX_ALL_SAVEING_DATA[h.reportguid]);
    ...
    for (var e = 0, g = d.length; e < g; e++) {
            ...
            for (var j in f) {
                    if (j == "WX_TYPE") {
                        ...
                        c += j + SAVING_NAMEVALUE_SEPERATOR + (f[j] == null ? "[W-X-N-U-L-L]" : f[j]) + SAVING_COLDATA_SEPERATOR
                    }
                }
...
function addAllCustomizeParamValues(b, e, d) {
    ...
    for (var c in a) {
        ...
        f = f + c + SAVING_NAMEVALUE_SEPERATOR + a[c] + SAVING_COLDATA_SEPERATOR
    }
...
function hasSaveData(a) {
    ...
    if (a.indexOf("_INSERTDATAS=") < 0 && a.indexOf("_UPDATEDATAS=") < 0 && a.indexOf("_DELETEDATAS=") < 0 && a.indexOf("_CUSTOMIZEDATAS=") < 0) {
        return false
    }
    return true
}
...
function getDeleteConfirmMessageAsString() {
    ...
    var b = "";
    for (var a = 0; a < WX_deleteConfirmessage.length; a++) {
        if (WX_deleteConfirmessage[a] == null || WX_deleteConfirmessage[a] == "" || WX_deleteConfirmessage[a] == "none") {
            return
        }
...
function loadAutoCompleteInputboxData(h, i) {
    ...
    g.inputboxid = d;
    sendAsynRequestToServer(a, fillAutoCompleteColsMethod, onGetAutoCompleteDataErrorMethod, g)
}
function fillAutoCompleteColsMethod(f, g) {
    ...
    var e = h != "" ? document.getElementById(getReportGuidByInputboxId(b) + "_tr_" + h) : null;
    if (e != null) {
...
function stopSaveForDemo() {
    wx_win("<div style='font-size:15px;color:#CC3399'><br>\u8fd9\u91cc\u662f\u516c\u5171\u6f14\u793a\uff0c\u4e0d\u5141\u8bb8\u4fdd\u5b58\u6570\u636e\u5230\u540e\u53f0<br><br>\u60a8\u53ef\u4ee5\u5728\u672c\u5730\u90e8\u7f72WabacusDemo\u6f14\u793a\u9879\u76ee\uff0c\u8fdb\u884c\u5b8c\u5168\u4f53\u9a8c\uff0c\u53ea\u9700\u51e0\u6b65\u5373\u53ef\u90e8\u7f72\u5b8c\u6210<br><br>WabacusDemo.war\u4f4d\u4e8e\u4e0b\u8f7d\u5305\u7684samples/\u76ee\u5f55\u4e2d</div>", {
        lock: true
    });
    return false
}
