 ...
 * 常用功能函数
 ********************************************/
function setExColValue(paramsObj)
{
	var pageID = paramsObj.pageid;

	var sreports = paramsObj.savingReportIds;
	for (var i=0;i<sreports.length;i++)
	{
		var jValue = {};
		var report = sreports[i];
		var colId = pageID+"_guid_"+report.reportid+"_excolmun";
        var reportCols = $("[id="+colId+"]"); 
        if (reportCols == null || reportCols.length < 1)
        {
        	continue;
        }
        for (var j=0;j<reportCols.length;j++)
        {
        	var hasValue = jValue[reportCols[j].name];
        	if (reportCols[j].type == "checkbox" && !reportCols[j].checked)
        	{
        		continue;
        	}
        	if (reportCols[j].type == "radio" && !reportCols[j].checked)
        	{
        		continue;
        	}
        	
        	if (hasValue == null || hasValue.length <1)
        	{
        		hasValue = "";
        	}else
        	{
        	...
        	jValue[reportCols[j].name]= hasValue+reportCols[j].value;
        }
    	setCustomizeParamValue(pageID,report.reportid,"EXT_COLVALUE",JSON.stringify(jValue))
	}
}

function refreshParentComponentDisplay(pageid,reportid)
{
	if(WXConfig.prompt_dialog_type=='ymprompt')
	{
		parent.refreshComponentDisplay(pageid,reportid,true);
	}else
	{
		artDialog.open.origin.refreshComponentDisplay(pageid,reportid,true);
	}
}


function setUploadColValue(pageid,reportid,uploadcolmun,btnHTML)
{
	if (uploadcolmun == null || uploadcolmun.length<1 )
	{
		return;
	}
	
	var uploadHTML = btnHTML[uploadcolmun];
	
	uploadHTML =  uploadHTML.replace(new RegExp("#DYH#","gm"),"'");  
	
	if (uploadcolmun.indexOf("diviamageshow_")<0)
	{

		btnHTML[uploadcolmun] = uploadHTML;
		//刷新单个字段
		if(WXConfig.prompt_dialog_type=='ymprompt')
		{
			//parent.setEditableReportColValue(pageid,reportid,btnHTML,null);
			parent.setEditableReportColValue(pageid,reportid,btnHTML,{name:uploadcolmun,value:bizid,matchmode:"equals"});
		}else
		{
			//artDialog.open.origin.setEditableReportColValue(pageid,reportid,btnHTML,null);
			artDialog.open.origin.setEditableReportColValue(pageid,reportid,btnHTML,{name:btnHTML.bizkey,value:btnHTML.bizid,matchmode:"equals"});

		}

	}else
	{
		var nIndex = uploadHTML.indexOf("<input");
		if (nIndex >0)
		{
			uploadHTML = uploadHTML.substring(0,nIndex);
			if(WXConfig.prompt_dialog_type=='ymprompt')
			{
				parent.document.all.getElementById(uploadcolmun).innerHTML = uploadHTML; 
			}else
			{
				artDialog.open.origin.document.getElementById(uploadcolmun).innerHTML = uploadHTML; 
			}
...
function openFullDialog(src,srcName,isscroll){
    var width=screen.width;
    var height=screen.height;
    var isscr = isscroll || "no";
    //window.showModalDialog(src,"","dialogWidth="+width+"px;dialogHeight="+height+";status=no;scrollbars=no");
    var new_window = window.open (src, 'newwindow', 'height='+height+', width='+width+',top=0,left=0,toolbar=no,menubar=no,scrollbars='+isscr+',resizable=no,location=no,status=no');
 ...
 * 
 */
function updateProjectHasFile(ctx,annex_id,hasfile){
	var hasFiled = "0";
	if (hasfile == true)
	{
		hasFiled = "1";
	...
	$.ajax({
		type : "post",
		datatype : "json",
		url : ctx+"/sdm/updateProjectHasFile.htm?annex_id="+annex_id+"&hasfile="+hasFiled,
 ...
 * 
 */
function deleteFileByFileID(pageid,reportid,ctx,fileid){
	wx_confirm("是否删除此文件？","","","",function(){
		$.ajax({
			type : "post",
			datatype : "json",
			url : ctx+"upload/deleteFileByFileID.htm?fileid="+fileid,
			success : function(responsedata) {
				//refreshComponentDisplay(pageid,reportid);
				document.getElementById("tr_"+fileid).style.display="none";
			}
		});
	});
}
...
//自动显示按钮,任务处理
function showAutoBtn(strParam)
{
	var strBtns = strParam.btns;
	var lisBtn = strBtns.split(",");
	for (var i =0;i<lisBtn.length;i++)
	{
		if (document.getElementById(lisBtn[i]) != null)
		{
			document.getElementById(lisBtn[i]).style.display="";
		}
	}
}
 ...
 * @param paramsObj
 */
function refreshParentForm(paramsObj) {

	var strHref = artDialog.open.origin.location.href;
	if (strHref == null || strHref.length<10)
	{
	  return;
	}
	strHref = strHref.replace(new RegExp("#","g"),"");

	var nIndex = strHref.indexOf("PAGEID=");
	if (nIndex >0)
	{
		var nEndIndex = strHref.indexOf("&",nIndex);
		if (nEndIndex < 0 )
		{
			nEndIndex = strHref.length;
		}
		var strPageID = strHref.substring(nIndex+7,nEndIndex);
		var curHref = artDialog.open.origin.document.getElementById(strPageID+"_url_id");
		if (curHref != null && curHref.getAttribute('value') != null && curHref.getAttribute('value').length>10)
		{
			strHref = curHref.getAttribute('value');
		}
	}
	//strHref = strHref.replace(new RegExp("RECORDCOUNT","g"),"RECORDCOUNT1");
	//strHref = strHref.replace(new RegExp("PAGECOUNT","g"),"PAGECOUNT1");
	
	setTimeout(function(){postParentURL(strHref);},1*1000);
}

//刷新父画面并刷新左边的树菜单
function refreshParentFormHasTree(paramsObj)
{
	refreshParentForm(paramsObj);
	refreshZTree();
}
...
{
	//判断此画面是否有树需要刷新
	if(artDialog.open.origin != null && artDialog.open.origin.parent != null && artDialog.open.origin.parent.refreshZTree != null)
	{
		artDialog.open.origin.parent.refreshZTree();
		return;
	}
}

//排序方面的回调
function sortCommCallBack(res)
{
	if (sortCallBack != null)
	{
		res = sortCallBack(res);
		if (res)
		{
			cmsUpdateCache(res);			
		}	
 ...
 * JS更新缓存
 */
function cmsUpdateCache(res){
	$.ajax({
		type : "post",
		datatype : "json",
		data:res,
		url : "cms/updateCache.htm",
		success : function(responsedata) {
		}
	});
}
...
/**
 * 更新Solr索引，不传ID则用时间增量更新，传了ID则只更新一条数据
 */
function refreshSolrIndex(indexType,indexId){
	$.ajax({
		type : "post",
		datatype : "json",
		url : "rdp/solr/refreshIndex.htm?indexType="+indexType+"&indexId="+indexId,
		success : function(responsedata) {
		}
	});
}
 ...
 * 收起、展开报表展示
 */
function foldedReport(pageId,reportId)
{
	var reportTitleId = pageId+"_guid_"+reportId+"_title";
	var reportDataId = pageId+"_guid_"+reportId+"_data";
	var foldedBtnId = pageId+"_guid_"+reportId+"_foldedBtn";
	if ($("#"+reportDataId).length>0)
	{
		var dataDisplay = $("#"+reportDataId)[0].style.display;
		if (dataDisplay == "none")
		{
			$("#"+reportDataId)[0].style.display = "";
			$("#"+reportTitleId).css("border-bottom","");
			$("#"+foldedBtnId).removeClass();
			$("#"+foldedBtnId).addClass("cls-table-folded-show");

		}else
		{
			$("#"+reportDataId)[0].style.display = "none";
			$("#"+reportTitleId).css("border-bottom","2px solid");
			$("#"+reportTitleId).css("border-bottom-color","#AAA");
			$("#"+foldedBtnId).removeClass();
			$("#"+foldedBtnId).addClass("cls-table-folded-hide");

		}
	}	
}

function postParentURL(url)
{
	...
	if (urlArr.length > 1)
	{
		...
		for (var i=0;i<urlParams.length;i++)
		{
			...
			if (urlParam.length>1)
			{
				...
				inputField.name=urlParam[0];
				inputField.value= decodeURI(urlParam[1]);
				if (urlParam[0] == "PAGEID" || urlParam[0] == "DISPLAY_TYPE")
				{
					if (postParam.length<1)
					{
						postParam = urlParam[0] +"="+urlParam[1];
				...
				{
					if (urlParam[0].indexOf("RECORDCOUNT") >0 || urlParam[0].indexOf("PAGECOUNT") >0)
					{

					}else if (urlParam[0].indexOf("PAGENO") >0)
					{
						var dynPageNo = urlParam[0].replace(new RegExp("PAGENO","g"),"DYNPAGENO");
						//如果当前的动作是新增，则转到第一页
						if (location.href.indexOf("WX_EDITTYPE=add")>0)
						{
							inputField.value= "1";
						}

						inputField.name=dynPageNo;
						formredwin.appendChild(inputField);
    ...
    formredwin.action = urlArr[0]+"?"+postParam;

    artDialog.open.origin.document.body.appendChild(formredwin);
    formredwin.submit();
}

/****************工作流处理**************************/
function submitWorkFlow(page,report,action,btntype,workFlowId,taskId)
{
	if (btntype == "flowimage")
	{
		var strURL = "rdp/activiti/showImage.htm?taskID="+taskId;
		wx_winpage(strURL, {initsize:'max'});
		return;
	}
	
	var actionBtnStr = "btn_"+page+"_guid_"+report+"_"+action+"_id";
	//设置递交变量
	var actionBtn = document.getElementById(actionBtnStr);
	setCustomizeParamValue(page,report,"workFlowID",workFlowId);
	setCustomizeParamValue(page,report,"taskID",taskId);
	setCustomizeParamValue(page,report,"workFlowControl",btntype);
	
	//设置审批意见
	var workflowsug = document.getElementById("__workflowsug");
	if (workflowsug != null)
	{
		var strworkflowsug = workflowsug.value;
		setCustomizeParamValue(page,report,"__workflowsug",strworkflowsug.replace(/\"/g,"\\\""));
	}
	
	//选择回退动作，传入回退的结点值
	if (btntype == "back")
	{
		var activeId = document.getElementById("__workflowbacks").value;
		setCustomizeParamValue(page,report,"__workflowbackactive",activeId);
	}
