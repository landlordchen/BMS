...
//检查列表数据重复共通方法
function checkListUnique(pageId,reportId,data,msg){
	...
	var flag = true;
	var datasArr= getEditableReportColValues(pageId,reportId,data,null);
	...
	if(datasArr!=null){
		for(var i =0;i<datasArr.length;i++){
			var uniqurStr = "";
			 ...
			 for(var key in data){
		 		uniqurStr = uniqurStr + datasArr[i][key].value;
		     }
		     
		     if(uniqueArr.indexOf(uniqurStr)>=0){
		     	wx_warn(msg);
		     	flag = false;
		     	break;
		     }else{
			     uniqueArr.push(uniqurStr); 
		     }
		}
	}
